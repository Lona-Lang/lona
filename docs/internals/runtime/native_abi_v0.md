...
当前实现已经在这套主规则之上补了两层保守优化：

- 固定布局且总大小为 `1 / 2 / 4 / 8` byte 的聚合
- 可以走“单寄存器聚合打包”直传 / 直返
- 固定布局且总大小不超过 `16` byte 的聚合返回
- 可以走“直接聚合返回”而不是 `sret`
...
- `单寄存器聚合打包`：把聚合压成一个整数寄存器，例如 `{i32, i32} -> i64`
- `直接聚合返回`：返回值仍按原聚合类型直接返回，只是去掉隐藏 `sret`

本文不把这两者和 LLVM 函数体内的 `SROA` pass 混用。
...
如果命中了上面的优化分支，则不走这条默认规则：

- `1 / 2 / 4 / 8` byte 的固定布局聚合可走单寄存器聚合打包
- `<= 16` byte 的固定布局聚合返回可直接返回
...
如果 `Point` 的 ABI 大小满足单寄存器聚合打包条件，则会变成：
...
```text
void @make_big(ptr sret(%Point))
```

小聚合直接返回示例：

```text
%Triple @make_small(ptr)
```
...
- 聚合默认间接
- 小固定布局聚合允许单寄存器聚合打包
- `<= 16` byte 的固定布局聚合返回允许 direct return
- 未命中 direct return 的聚合返回统一 `sret`
- 只支持 `Linux x86_64`
